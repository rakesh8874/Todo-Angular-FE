...
import { Component, HostBinding, resolveForwardRef } from '@angular/core';
...
//  injecting form builder and todo service
  constructor(private fb:FormBuilder, private todo:TodoServiceService,
  ...
  getAllCategory(){
    this.todo.getAllCategories().subscribe(
      response=>{
          ...
          if(this.allCategoryDetails.length === 0 ){

            this.showWelcomeMsg = true;

          }else if(this.allCategoryDetails.length !== 0){
          ...
          if(this.pageSlice.length === 0 && this.showCategoryContainer){
